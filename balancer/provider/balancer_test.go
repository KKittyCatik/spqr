...
func TestMaxFitOnShard(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedCount: 64, // cpu: 0.8*((100-20)/(10/10)) = 64, space: 0.8*((1000-200)/(100/10)) = 64, returns highest capacity from any metric
		},
		{
		...
		},
		{
			...
			expectedCount: 80, // cpu: 0.8*((1000-900)/(10/10)) = 80, space: 0.8*((100-50)/(10/10)) = 40, returns highest capacity from any metric
		},
		{
			...
			expectedCount: 128, // cpu: 0.8*((200-100)/(20/10)) = 40, space: 0.8*((500-100)/(25/10)) = 128, returns highest capacity from any metric
		},
	}
